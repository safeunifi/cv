...
import { angleBetweenThreePoints, angleFromVertical, evaluateAngle } from './geometry';
import { recommendDrills } from './drill-recommendation';
...
export function analyzeSwing(
  ...
  return { phases: phaseAnalyses, overallScore, faults, strengths, recommendations, tempo };
}

function identifyPhases(frames: PoseFrame[]): Partial<Record<SwingPhase, PoseFrame[]>> {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (maxW > maxHandY) { maxHandY = maxW; topIdx = i; }
  }
  ...
  for (let i = topIdx; i < total; i++) {
    ...
    if (minW < minHandY) { minHandY = minW; impactIdx = i; }
  }
...
function analyzePhase(phase: SwingPhase, frames: PoseFrame[], cameraAngle: CameraAngle): PhaseAnalysis {
  ...
  switch (phase) {
    case 'address': observations.push(...analyzeAddress(angles)); break;
    case 'topOfBackswing': observations.push(...analyzeTop(angles)); break;
    case 'impact': observations.push(...analyzeImpact(angles)); break;
    case 'finish': observations.push(...analyzeFinish(angles)); break;
  }

  if (observations.length === 0) observations.push('Position looks solid');

  return { score: Math.max(0, Math.min(100, score)), keyAngles: angleRecord, observations, frameTimestamp: keyFrame.timestamp };
}

export function calculateAngles(frame: PoseFrame, cameraAngle: CameraAngle): SwingAngles {
  const j = frame.joints;
  const angles: SwingAngles = {};

  if (j.root && j.neck) angles.spineAngle = angleFromVertical(j.root, j.neck);
  if (j.leftHip && j.leftKnee && j.leftAnkle) {
    angles.kneeFlex = 180 - angleBetweenThreePoints(j.leftHip, j.leftKnee, j.leftAnkle);
  }
  if (j.neck && j.root && j.leftKnee) angles.hipBend = angleBetweenThreePoints(j.neck, j.root, j.leftKnee);
  ...
  if (j.leftElbow && j.leftWrist) angles.wristHinge = angleFromVertical(j.leftElbow, j.leftWrist);
  if (j.nose) angles.headMovement = Math.abs(j.nose.x - 0.5) * 10;
...
function analyzeImpact(a: SwingAngles): string[] {
  const obs: string[] = [];
  if (a.spineAngle != null && a.spineAngle < 20) obs.push('Early extension detected. Maintain your spine angle through impact.');
  if (a.headMovement != null && a.headMovement > 3) obs.push('Significant head movement. Focus on keeping your head steady.');
  return obs;
}

function analyzeFinish(a: SwingAngles): string[] {
  const obs: string[] = [];
  if (a.shoulderTurn != null && a.shoulderTurn < 160) obs.push('Incomplete follow-through. Rotate fully to a balanced finish.');
  return obs;
}
...
function detectFaults(
  ...
  if (addressSpine != null && impactSpine != null) {
    ...
    if (loss > 10) {
      faults.push({
        id: uuid(), name: 'Early Extension',
        severity: loss > 20 ? 'Major' : 'Moderate', phase: 'downswing',
  ...
  if (cameraAngle === 'faceOn' && frames.length > 10) {
    ...
    if (sway > 0.05) {
      faults.push({
        id: uuid(), name: 'Hip Sway',
        severity: sway > 0.10 ? 'Major' : 'Moderate', phase: 'backswing',
  ...
  if (topWrist != null && midWrist != null && topWrist - midWrist > 30) {
    faults.push({
      id: uuid(), name: 'Casting / Early Release',
      severity: 'Major', phase: 'downswing',
  ...
  if (shoulderTurn != null && shoulderTurn < 80) {
    faults.push({
      id: uuid(), name: 'Restricted Turn',
      severity: shoulderTurn < 70 ? 'Major' : 'Moderate', phase: 'backswing',
  ...
  if (impactArm != null && impactArm < 160) {
    faults.push({
      id: uuid(), name: 'Chicken Wing',
      severity: impactArm < 140 ? 'Major' : 'Moderate', phase: 'impact',
  ...
  if (headMove != null && headMove > 3) {
    faults.push({
      id: uuid(), name: 'Head Movement',
      severity: headMove > 5 ? 'Major' : 'Minor', phase: 'downswing',
...
function identifyStrengths(phaseAnalyses: Partial<Record<SwingPhase, PhaseAnalysis>>): string[] {
  const strengths: string[] = [];
  if ((phaseAnalyses.address?.score ?? 0) >= 80) strengths.push('Good athletic posture at address');
  if ((phaseAnalyses.topOfBackswing?.score ?? 0) >= 80) strengths.push('Solid position at the top of backswing');

  const st = phaseAnalyses.topOfBackswing?.keyAngles['Shoulder Turn'];
  if (st != null && st >= 85 && st <= 100) strengths.push(`Excellent shoulder turn (${Math.round(st)}°)`);

  const xf = phaseAnalyses.topOfBackswing?.keyAngles['X-Factor'];
  if (xf != null && xf >= 35 && xf <= 55) strengths.push(`Great X-Factor separation (${Math.round(xf)}°)`);

  if ((phaseAnalyses.impact?.score ?? 0) >= 80) strengths.push('Strong impact position');

  const la = phaseAnalyses.topOfBackswing?.keyAngles['Lead Arm'];
  if (la != null && la >= 170) strengths.push('Excellent lead arm extension');

  if (strengths.length === 0) strengths.push('Committed swing with good effort');
  return strengths;
}

function calculateTempo(phaseFrames: Partial<Record<SwingPhase, PoseFrame[]>>): SwingTempo {
  ...
  return { backswingDuration: backDur, downswingDuration: downDur, totalDuration: totalDur, ratio: downDur > 0 ? backDur / downDur : 0 };
}
...
function calculateOverallScore(
  ...
  const weights: Record<SwingPhase, number> = {
    address: 0.10, takeaway: 0.05, backswing: 0.15, topOfBackswing: 0.20,
    downswing: 0.15, impact: 0.25, followThrough: 0.05, finish: 0.05,
  };
...
function createMinimalAnalysis(): SwingAnalysis {
  return {
    ...
    overallScore: 0,
    faults: [{
      id: uuid(), name: 'Insufficient Data', severity: 'Minor', phase: 'address',
