import { PoseFrame, JointName } from '@/types/golf';

/**
 * Generates simulated pose data for swing analysis.
 *
 * NOTE: For production, integrate one of:
...
export async function extractPoseFrames(
  ...
  for (let i = 0; i < numFrames; i++) {
    ...
    frames.push({ timestamp: timeMs / 1000, joints: pose });
  }
...
function generatePoseForFrame(
  ...
  if (progress < 0.10) {
    ...
    const backProg = (progress - 0.10) / 0.35;
